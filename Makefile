...
# The wordmark, the icon and the share image are set in a monospace face;
# Courier New resolves through fontconfig to Liberation Mono, which is
# metric-compatible, so the generated assets match what the site renders.

CHROME   ?= google-chrome
MAGICK   ?= magick
INKSCAPE ?= inkscape

# The sheet colour, without the leading hash: an unescaped '#' in a make
# assignment starts a comment. Icons are flattened onto the mark's own
# graphite rather than kept transparent, so they read the same on a light
# or a dark browser chrome.
SHEET    := e6eae4
GRAPHITE := 20242a
...
og: ## Re-render og.png from tools/og.html
	$(CHROME) --headless=new --disable-gpu --hide-scrollbars \
		--force-device-scale-factor=1 --default-background-color=$(SHEET)FF \
		--screenshot=/tmp/bwm-og-raw.png --window-size=1200,630 tools/og.html
	$(MAGICK) /tmp/bwm-og-raw.png -strip -dither None -colors 128 PNG8:og.png
	@rm -f /tmp/bwm-og-raw.png
	@echo "wrote og.png"
...
favicon: ## Outline favicon.svg, then re-render favicon.ico and apple-touch-icon.png
	...
	$(MAGICK) -background "#$(GRAPHITE)" favicon.svg -resize 180x180 \
		-alpha off -colorspace sRGB -type TrueColor -strip apple-touch-icon.png
	$(MAGICK) \
		\( -background "#$(GRAPHITE)" favicon.svg -resize 48x48 \) \
		\( -background "#$(GRAPHITE)" favicon.svg -resize 32x32 \) \
		\( -background "#$(GRAPHITE)" tools/favicon-16.svg -resize 16x16 \) \
